...
frappe.ui.form.on("Leave Application", {
	 leave_approver : function(frm) {
		// console.log(frm.doc.leave_approver);
		// if (frm.doc.leave_approver) {			
		// 	employee_name=frappe.db.get_value('Employee', {'user_id': frm.doc.leave_approver}, ['employee_name'])
		...
		if (frm.doc.docstatus === 0 && frm.doc.employee && frm.doc.leave_type && frm.doc.from_date && frm.doc.to_date) {
			...
			return frappe.call({
				...
				callback: function (r) {
					...
					if(total_leave_balance < leave_days && frm.doc.leave_type != 'Other Leave'){
	...
	get_leave_balance: function(frm) {
		if (frm.doc.docstatus === 0 && frm.doc.employee && frm.doc.leave_type && frm.doc.from_date && frm.doc.to_date) {
			...
			return frappe.call({
				...
				callback: function (r) {
					if(frm.doc.leave_type == 'Other Leave'){
	...
	onload: function(frm) {
			...
			if (!frm.is_new()) {
				...
				//for the testing pupose use this users for leave application status change
				if(frappe.session.user == 'hrmanager@iiti.ac.in'){
					frm.toggle_display("submit_form", false);
	...
	leave_type:function(frm){
		...
		set_leave_authority(frm);
		if(frm.doc.leave_type == 'Other Leave'){
			frm.set_query("other_leave_type", function() {
				return {
					filters: {
						status:['=','Active']
					}

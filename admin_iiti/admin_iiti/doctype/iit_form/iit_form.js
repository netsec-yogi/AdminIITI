...
frappe.ui.form.on('IIT Form', {
	refresh: function(frm) {
		...
		let approver_details = frm.doc.approvers;
		let approved_count = frm.doc.approvers.length;
		if(approver_details){
			approver_details.forEach(function (list) {
				if(list.approver == logged_user && list.status == 'Open' && frm.doc.status != 'Recommended' && approved_count >1 ){
				...
				if(list.approver == logged_user && list.status == 'Open' && approved_count == 1){
					frm.add_custom_button(__('Approve'), function(){
						let action_type = 'Approved';
						change_form_status(frm,action_type,logged_user,list.idx);
					}).addClass("btn-info").css({'color':'#ffffff','font-weight': 'bold','background-color': '#6495ed'})
...
function change_form_status(frm,action_type,approver_user,approved_count) {
	frappe.call({
		...
		"args": {
			...
			"total_approver_count" :frm.doc.approvers.length,
